...
with st.form("prediction_form"):
    ...
    with col1:
        ...
        sk_id_curr = st.number_input("SK_ID_CURR", value=100000)
        years_birth = st.number_input("YEARS_BIRTH (Age)", value=30.0)
        years_employed = st.number_input("YEARS_EMPLOYED", value=5.0)
        years_registration = st.number_input("YEARS_REGISTRATION", value=5.0)
        years_id_publish = st.number_input("YEARS_ID_PUBLISH", value=5.0)
        cnt_fam_members = st.number_input("CNT_FAM_MEMBERS", value=1.0)
        ...
        region_pop_relative = st.number_input("REGION_POPULATION_RELATIVE", value=0.0188, format="%.6f")

    with col2:
        st.subheader("Financial & External")
        amt_income_total = st.number_input("AMT_INCOME_TOTAL", value=150000.0)
        amt_credit = st.number_input("AMT_CREDIT", value=400000.0)
        amt_annuity = st.number_input("AMT_ANNUITY", value=20000.0)
        amt_goods_price = st.number_input("AMT_GOODS_PRICE", value=400000.0)
        ext_source_2 = st.number_input("EXT_SOURCE_2", value=0.5, format="%.6f")
        ext_source_3 = st.number_input("EXT_SOURCE_3", value=0.5, format="%.6f")
        ...
        amt_req_credit_bureau_year = st.number_input("AMT_REQ_CREDIT_BUREAU_YEAR", value=0.0)

    with col3:
        st.subheader("Process & Social")
        weekday_appr = st.text_input("WEEKDAY_APPR_PROCESS_START", value="MONDAY")
        hour_appr = st.number_input("HOUR_APPR_PROCESS_START", value=10)
    ...
    with exp_col1:
        prev_amt_annuity_mean = st.number_input("PREV_AMT_ANNUITY_MEAN", value=0.0)
        prev_amt_annuity_median = st.number_input("PREV_AMT_ANNUITY_MEDIAN", value=0.0)
        ...
        prev_cnt_payment_max = st.number_input("PREV_CNT_PAYMENT_MAX", value=0.0)
        prev_sellerplace_area_mean = st.number_input("PREV_SELLERPLACE_AREA_MEAN", value=0.0)
        prev_sk_id_prev_count = st.number_input("PREV_SK_ID_PREV_COUNT", value=0.0)
        prev_sk_id_curr_first = st.number_input("PREV_SK_ID_CURR_FIRST", value=0.0)

    with exp_col2:
        prev_years_decision_mean = st.number_input("PREV_YEARS_DECISION_MEAN", value=0.0)
        prev_years_first_due_mean = st.number_input("PREV_YEARS_FIRST_DUE_MEAN", value=0.0)
        prev_years_last_due_mean = st.number_input("PREV_YEARS_LAST_DUE_MEAN", value=0.0)
        prev_years_last_due_1st_version_mean = st.number_input("PREV_YEARS_LAST_DUE_1ST_VERSION_MEAN", value=0.0)
        prev_years_first_drawing_mean = st.number_input("PREV_YEARS_FIRST_DRAWING_MEAN", value=9999.0)
        prev_years_termination_mean = st.number_input("PREV_YEARS_TERMINATION_MEAN", value=0.0)
        prev_hour_appr_mean = st.number_input("PREV_HOUR_APPR_PROCESS_START_MEAN", value=0.0)
